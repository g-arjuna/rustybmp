"""
Layer 5 — Mixed FRR + XRd Multi-NOS Smoke Test
...
The test starts rustybmp as a host process, spins up one mixed-NOS
ContainerLab topology, waits for both FRR and XRd BMP senders to connect, then
verifies that speakers, peers, routes, and XRd stats are visible together via
the HTTP API.
"""
import os
import shutil
...
def rustybmp_server():
    ...
    log_path = log_dir / "layer5_mixed_rustybmp.log"
...
def clab_topology(rustybmp_server):
    try:
        ...
        if (
            ...
            pytest.skip(
                "Mixed lab images unavailable; ensure FRR 10.6.1 and "
...
def speakers_ready(api_ready):
    return _wait_for_json(
        "/api/speakers",
        "speakers",
        lambda speakers: isinstance(speakers, list) and len(speakers) >= 4,
    )
...
def peers_ready(speakers_ready):
    return _wait_for_json(
        ...
        lambda peers: (
            ...
            and len([peer for peer in peers if peer.get("state") == "up"]) >= 4
        ),
    )


@pytest.fixture(scope="module")
def routes_ready(peers_ready):
    return _wait_for_json(
        "/api/routes?limit=50&action=announce",
        "routes",
        lambda routes: isinstance(routes, list) and len(routes) >= 8,
    )
...
class TestMixedFrrXrd:
    ...
    def test_four_speakers_connected(self, speakers_ready):
        assert len(speakers_ready) >= 4

    def test_speakers_include_frr_and_xrd(self, speakers_ready):
        speaker_addrs = {speaker.get("addr") for speaker in speakers_ready}
        expected = {"172.20.22.2", "172.20.22.3", "172.20.22.4", "172.20.22.5"}
        assert expected.issubset(speaker_addrs), (
            f"Expected mixed speaker addresses {expected}, got {speaker_addrs}"
        )

    def test_four_peers_up(self, peers_ready):
        up_peers = [peer for peer in peers_ready if peer.get("state") == "up"]
        assert len(up_peers) >= 4

    def test_peer_asns_cover_both_nos_groups(self, peers_ready):
        peer_asns = {peer.get("peer_as") for peer in peers_ready}
        assert {65100, 65200}.issubset(peer_asns), (
            f"Expected FRR and XRd peer ASNs, got {peer_asns}"
        )

    def test_known_frr_prefix_present(self, routes_ready):
        routes = _wait_for_prefix("10.100.1.0/24")
        assert any(route.get("prefix") == "10.100.1.0/24" for route in routes)

    def test_known_xrd_prefix_present(self, routes_ready):
        routes = _wait_for_prefix("10.110.1.0/24")
        assert any(route.get("prefix") == "10.110.1.0/24" for route in routes)

    def test_routes_cover_all_router_prefix_families(self, routes_ready):
        expected = [
            "10.100.1.0/24",
            "10.200.1.0/24",
            "10.110.1.0/24",
        ...
        for prefix in expected:
            ...
            assert any(route.get("prefix") == prefix for route in routes), (
                f"Expected mixed FRR/XRd prefix {prefix} to be queryable"
            )

    def test_xrd_stats_visible_alongside_frr_routes(self, stats_ready):
        stat_types = {stat.get("stat_type") for stat in stats_ready}
        assert {7, 8, 9, 10}.intersection(stat_types), (
            f"Expected XRd legacy/global stats counters in mixed lab, got {stat_types}"
        )
